...
import { getProjectMailConfig, imapConfigured } from './project-mail-config.service';
...
export async function findThread(customerId: unknown, subject: string, references: string[], sender: string, projectId?: string): Promise<string> {
  ...
  if (customerId) {
    const outgoing = await DevelopmentLetter.findOne({ projectId: scopedProjectId, customerId, messageId: { $in: references } }).sort({ createdAt: -1 });
    if (outgoing) {
  ...
  const incoming = await MailMessage.findOne({ projectId: scopedProjectId, customerId: customerId || null, from: sender, messageId: { $in: references } }).sort({ sentAt: -1 });
  ...
  if (normalizeSubject(subject) && /^(re|fw|fwd|回复|答复|转发)\s*[:：]/i.test(subject.trim())) {
    ...
    if (customerId) {
      const outgoing = await DevelopmentLetter.find({ projectId: scopedProjectId, customerId, recipientEmail: sender, status: { $in: ['sent', 'opened'] }, sentAt: { $gte: new Date(Date.now() - 30 * 86400000) } }).sort({ sentAt: -1 }).limit(100);
...
export async function importMail(source: Buffer, account?: string, projectId?: string) {
  const scopedProjectId = await resolveProjectId(projectId);
  ...
  const matches = await Customer.find({ projectId: scopedProjectId, email: from }).limit(2).select('_id');
  ...
  const threadId = await findThread(customerId, parsed.subject || '', [...references, parsed.inReplyTo || ''], from, scopedProjectId);
...
async function syncProject(projectId: string, makeClient: ClientFactory): Promise<void> {
  const config = await getProjectMailConfig(projectId);
  if (!config.imap.enabled || syncing.has(projectId)) return;
  syncing.add(projectId);
  const key = await mailboxKeyForProject(projectId);
  ...
  try {
    ...
    client = makeClient({ host: config.imap.host!, port: config.imap.port, secure: config.imap.secure,
      doSTARTTLS: config.imap.secure ? undefined : true,
      auth: { user: config.imap.user!, pass: config.imap.password! }, logger: false,
    ...
    const lock = await client.getMailboxLock(config.imap.mailbox, { readOnly: true });
    try {
      ...
      for (const uid of uids) {
        ...
        try {
          ...
          if (meta && (meta.size ?? Infinity) <= env.IMAP_MAX_MESSAGE_SIZE) {
            ...
            if (data && data.source) await importMail(data.source, key, projectId);
          } else if (meta) {
...
export async function syncInbox(projectIdOrClient?: string | ClientFactory, clientFactory?: ClientFactory): Promise<void> {
  ...
  if (requestedId) return syncProject(await resolveProjectId(requestedId), makeClient);
