...
import { AgentAction, AgentMessage, AgentRun, AgentSession, Customer, CustomerAttachment, CustomerEvent, DevelopmentLetter, FollowUp, LetterTemplate, Project, Quotation, Scratchpad, User } from './models';
...
async function bootstrap(): Promise<void> {
  try {
    ...
    await Promise.all([Project.init(), User.init(), Customer.init(), DevelopmentLetter.init(), MailMessage.init(), MailSyncState.init(),
      ...
      AgentSession.init(), AgentMessage.init(), AgentRun.init(), AgentAction.init()]);
    const app = createApp();
