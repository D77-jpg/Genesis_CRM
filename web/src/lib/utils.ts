 ...
 * 生成浏览器侧请求标识。
 *
 * `crypto.randomUUID()` 在局域网 HTTP 等非安全上下文中可能不存在；优先使用它，
 * 其次使用仍可用的 `getRandomValues()` 生成 UUID v4，最后才使用非安全随机回退。
 * 此标识只用于发送请求幂等，不承担登录、授权或追踪 token 的安全职责。
 */
export function createClientId(): string {
  ...
  // RFC 4122 UUID v4 的版本位与 variant 位。
